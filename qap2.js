 ...
 *
 *      Name: Ellen Dalton
 *      Date: Oct. 12, 2023 - Oct. 18, 2023
 * 

1.	How many hours did it take you to complete this assessment? 
    (Please keep try to keep track of how many hours you have spent working on 
    each individual part of this assessment as best you can - an estimation is fine; 
    we just want a rough idea.)

Answer: [In total, I spent about 17 hours working on this assignment.]

2.	What online resources you have used? (My lectures, YouTube, Stack overflow etc.)

Answer: [I used W3Schools as well as ChatGPT.]

3.	Did you need to ask any of your friends in solving the problems. 
    (If yes, please mention name of the friend. They must be amongst your class fellows.)

Answer: [No.]

4.	Did you need to ask questions to any of your instructors? If so, how many questions 
    did you ask (or how many help sessions did you require)?

Answer: [No.]

5.	Rate (subjectively) the difficulty of Making this all! from your own perspective, 
    and whether you feel confident that you can solve a similar but different problem 
    requiring some of the same techniques in the future now that you’ve completed this one.

Answer: [I would rate this assignment as being a 7.5 out of 10 for difficulty. I do, 
        however, feel confident that I can solve similar but different problems in the 
        future, now that I have completed this one. Trying to complete this assignment
        has given me some much needed practise in JavaScript!]
...
function snake(value) {
    let noSpaceStart = value.replace(/^\s+/, "");
    let noSpaceEnd = noSpaceStart.replace(/\s+$/, "");
    let underscore = noSpaceEnd.replace(/\s+/g, "_");
    let noDots = underscore.replace(/\./g, "_");
    let noDoubleUnderscores = noDots.replace(/_+/g, "_");
    let lowercase = noDoubleUnderscores.toLowerCase();
    return lowercase;
}
...
console.log(snake(' A..  B   C '));
  ...
  function createVideo(src, width, controls) {
    let noSpaceStart = src.replace(/^\s+/, "");
    let noSpaceEnd = noSpaceStart.replace(/\s+$/, "");

    if (controls === true && width != undefined){
        let html = `<video src="${noSpaceEnd}" width="${width}" controls></video>`
        return html;
    } else if (controls === true && width === undefined){
        let html = `<video src="${noSpaceEnd}" controls></video>`
        return html;
    } else if (controls === undefined && width != undefined){
        let html = `<video src="${noSpaceEnd}" width="${width}"></video>`
        return html;
    } else {
        let html = `<video src="${noSpaceEnd}"></video>`
        return html;
    }
  };
...
function parseDateString(value) {
    ...
    if (!dateRegEx.test(value)){
        throw "The date you provided ("+value+") must be in the format YYYY-MM-DD";
    }
...
function parseDateStringAndCatchError(value) {
    try {
...
console.log(parseDateStringAndCatchError("2021-09-29"));
console.log(parseDateStringAndCatchError('01-01-01'));
console.log(parseDateStringAndCatchError('2021-1-01'));
console.log(parseDateStringAndCatchError("this is totally wrong"))
...
function toDateString(value) {
    let regex = /[A-Z][a-z][a-z][a-z]?[a-z]?\s[A-Z][a-z][a-z][a-z]?\s\d\d\s\d\d\d\d\s\d\d\:\d\d\:\d\d\s[A-Z][A-Z][A-Z]-\d\d\d\d\s(...)/
    let test = regex.test(value)
    if (test!=true){
        throw "ERROR: The value you provided (i.e. "+value+") is incorrect and must be in date object format."
    } else {
        let dateString = value.toDateString();
        let array = dateString.split(" ");
        if (array[1] === "Jan"){
            return `${array[3]}-01-${array[2]}`
        } else if (array[1] === "Feb"){
            return `${array[3]}-02-${array[2]}`
        } else if (array[1] === "Mar"){
            return `${array[3]}-03-${array[2]}`
        } else if (array[1] === "Apr"){
            return `${array[3]}-04-${array[2]}`
        } else if (array[1] === "May"){
            return `${array[3]}-05-${array[2]}`
        } else if (array[1] === "Jun"){
            return `${array[3]}-06-${array[2]}`
        } else if (array[1] === "Jul"){
            return `${array[3]}-07-${array[2]}`
        } else if (array[1] === "Aug"){
            return `${array[3]}-08-${array[2]}`
        } else if (array[1] === "Sept"){
            return `${array[3]}-09-${array[2]}`
        } else if (array[1] === "Oct"){
            return `${array[3]}-10-${array[2]}`
        } else if (array[1] === "Nov"){
            return `${array[3]}-11-${array[2]}`
        } else {
            return `${array[3]}-12-${array[2]}`
        }
    }
}
...
try{
  ...
  function normalizeCoord(value) {
    const coordinateFormat = "(\\-)?(\\d)*(\\.)?(\\d)*"
    const format1 = new RegExp("^" + coordinateFormat + "\," + coordinateFormat + "$")
    const format2 = new RegExp("^\\[" + coordinateFormat + ", " + coordinateFormat + "\\]$")

    let removeBrackets = value.replace(/^\[/g, "")
    let removeBrackets2 = removeBrackets.replace(/\]$/g, "")
    let removeSpaces = removeBrackets2.replace(/\s+/g, "")
    let split = removeSpaces.split(",")
    if(split.length === 2){
        let value1 = parseFloat(split[0])
        let value2 = parseFloat(split[1])
        
        let lat = value1
        let lng = value2
        if (format1.test(value)) {
            // do nothing because values already set above
        } else if (format2.test(value)) {
            lat = value2
            lng = value1
        } else { 
            throw new Error("Input does not match valid format") 
        }

        if(-90<=lat && lat<=90 && -180<=lng && lng<=180) {
            return `(${lat}, ${lng})`
        } else {
            throw new Error("Input is not a valid geographic coordinate")
        }
    };
  ...
  function formatCoords(...values) {
    ...
    values.forEach(value => {
        try {
            ...
            cleanedValues.push(normalizedCoord)
        }
        catch (e) {
            // catch quietly because problem requires invalid coordinates be ignored
            // console.log("Error: Could not parse value "+value)
        }
    })
    // console.log(cleanedValues);
    let joinedValues = cleanedValues.join(", ");
    return `(${joinedValues})`
  }
...
function mimeFromFilename(filename) {
    let regex = /(\.?[a-z][a-z]\d?[a-z]?[a-z]?)$/
    let test = regex.test(filename)
    // console.log(test)
    let fileExtension = filename.match(regex);
    // console.log(fileExtension);
    if (test === true){
        switch (fileExtension[0] || fileExtension[1]) {
            case ".html":
            case ".htm":
            case "html":
            case "htm":
...
console.log(mimeFromFilename("hello.html"));
console.log(mimeFromFilename(".htm"));
console.log(mimeFromFilename(".css"));
console.log(mimeFromFilename(".js"));
console.log(mimeFromFilename(".jpg"));
console.log(mimeFromFilename(".jpeg"));
console.log(mimeFromFilename(".gif"));
console.log(mimeFromFilename(".bmp"));
console.log(mimeFromFilename(".ico"));
console.log(mimeFromFilename(".cur"));
console.log(mimeFromFilename(".png"));
console.log(mimeFromFilename(".svg"));
console.log(mimeFromFilename(".webp"));
console.log(mimeFromFilename(".mp3"));
console.log(mimeFromFilename(".wav"));
console.log(mimeFromFilename(".mp4"));
console.log(mimeFromFilename(".webm"));
console.log(mimeFromFilename(".json"));
console.log(mimeFromFilename(".mpeg"));
console.log(mimeFromFilename(".csv"));
console.log(mimeFromFilename(".ttf"));
console.log(mimeFromFilename(".woff"));
console.log(mimeFromFilename(".zip"));
console.log(mimeFromFilename(".avi"));
console.log(mimeFromFilename("unknown.extension"));
console.log(mimeFromFilename("noExtension"));
  ...
  function generateLicenseLink(licenseCode, targetBlank) {
    let split = licenseCode.split("-");
    if (split.length === 2){
        let newString = split[1].toLowerCase()
        let url = `https://creativecommons.org/licenses/${newString}/4.0/`
        return url
    } else if (split.length === 3){
        let newString = (split[1] + "-" + split[2]).toLowerCase()
        let url = `https://creativecommons.org/licenses/${newString}/4.0/`
        return url
    } else if (split.length === 4){
        let newString = (split[1] + "-" + split[2] + "-" + split[3]).toLowerCase()
        let url = `https://creativecommons.org/licenses/${newString}/4.0/`
        return url
    } else {
        throw new Error("License code is not in a valid format.")
    }
...
function pureBool(value) {
    const regex1 = /^(yes|y|oui|o|t|true|vrai|v|[1-9]\d*)$/i;
    let test1 = regex1.test(value);

    const regex2 = /^(no|non|n|f|false|faux|0|-\d+)$/i
    let test2 = regex2.test(value);
...
function buildUrl(query, order, count, license) {
    ...
    if (1 <= count && count <= 50){
        ...
        throw new Error("Count (the number of results per page) must be between 1 and 50");
    }

    if (order === 'ascending' || order === 'descending'){
        o = `order=${order}`;
    } else {
        throw new Error("Order must be one of 'ascending' or 'decending'");
    }
        
    if (license === 'none' || license === 'any' || license === 'cc-by' || license === 'cc-by-nc' || license === 'cc-by-sa' || license === 'cc-by-nd' || license === 'cc-by-nc-sa' || license === 'cc-by-nc-nd'){
        l = `license=${license}`;
    } else {
        throw new Error("License must be one of 'none', 'any', 'cc-by', 'cc-by-nc', 'cc-by-sa', 'cc-by-nd', 'cc-by-nc-sa', 'cc-by-nc-nd'")
    }

    let url = `https://api.inaturalist.org/v2/observations?${q}&${c}&${o}&${l}`
    return url;
}
